...
function resetMaps() {
  ...
  // توسط میلاد کامنت شده این بخش بعدا می‌تواند خذف شود
  /*mapfuncs["Random"] = {
...
function Map() {
  ...
  this.canscroll = true;
  this.floor = 104;
  this.time = 500; // optionally specified later
  this.curloc = -1;
...
function getAreaFillStyle(setting) {
  var bgColor = "#5c94fc"; // پیش‌فرض: رنگ آبی آسمان ماریو
  
  if(stringHas(setting, "Underworld") ||
     stringHas(setting, "Castle") ||
     stringHas(setting, "Night")) {
      bgColor = stringHas(setting, "Underwater") ? "#2038ec" : "black"; // مشکی برای زیرزمین و قلعه
  } else if(stringHas(setting, "Underwater")) {
      bgColor = "#2038ec"; // آبی پررنگ برای زیر آب
  }
  
  // دستور مستقیم به مرورگر برای تغییر رنگ کل فضای سایت (بالا و پایینِ بوم نقاشی)
...
//  setMap();
//}
//پایینی‌رو با بلایی جابجا کردم

function endLevel() {
  if(map.ending) return;
  map.ending = true;
  
  // ۱. متوقف کردن روند بازی
  pause();

  // ۲. ساختن صفحه سیاه با کدهای HTML
  ...
  overlay.style.zIndex = "10000"; // برای اینکه روی همه چیز (حتی خود بازی) قرار بگیرد
  ...
  nameText.innerText = "Milad Maddah";
  ...
  contactHeader.style.color = "#ffcc00"; // رنگ زرد سکه‌های ماریو
  contactHeader.style.marginBottom = "30px";

  // ۵. اطلاعات تماس و تخصص شما
  var contactInfo = document.createElement("div");
  contactInfo.style.fontSize = "20px";
  contactInfo.style.lineHeight = "2";
  contactInfo.style.textAlign = "center";
  contactInfo.style.marginBottom = "50px";
  
  // نکته: ایمیل و شماره تماس دقیق خودت را در خط زیر جایگزین کن
  contactInfo.innerHTML = "<br>Email: maddah.milad@gmail.com<br>Phone: +98 919 819 3089";
  ...
  resetBtn.style.backgroundColor = "#e52521"; // رنگ قرمز کلاه ماریو
  resetBtn.style.color = "white";
  resetBtn.style.border = "4px solid white";
  resetBtn.style.borderRadius = "10px";
  resetBtn.style.fontFamily = "inherit";
  resetBtn.style.fontWeight = "bold";
  
  // افکت تغییر رنگ هنگام رفتن موس روی دکمه
  resetBtn.onmouseover = function() { resetBtn.style.backgroundColor = "#cc0000"; };
  resetBtn.onmouseout = function() { resetBtn.style.backgroundColor = "#e52521"; };

  // ۷. اتفاقی که با کلیک روی دکمه می‌افتد
  resetBtn.onclick = function() {
    // پاک کردن صفحه سیاه از روی مرورگر
    document.body.removeChild(overlay);
    
    // بازگرداندن بازی به مرحله اول
    window.currentmap = [1, 1]; 
    storePlayerStats();
    window.paused = false;
    setMap();
  ...
  // ۸. چسباندن همه این المان‌ها به صفحه سیاه و نمایش آن
  overlay.appendChild(nameText);
  overlay.appendChild(contactHeader);
...
var createUnderworld = function(locGen, exitLoc, sectionKey) {
    return new Area("Underworld", function() {
      setLocationGeneration(locGen);
      
      // ۱. بازگرداندن اتاق زیرزمین به ابعاد کاملاً ارجینال ماریو
      makeCeiling(32, 7);
      pushPreFloor(0, 0, 17);
      ...
      // ۲. لوله خروج دقیقاً سر جای اولش
      pushPreThing(PipeSide, 104, 16, exitLoc);
      pushPreThing(PipeVertical, 120, 88, 88);

      // ۳. قرار دادن متن‌های رزومه در فضای خالی و کاملاً سیاه سمت راستِ لوله
      if (sectionKey && typeof myResumeData !== 'undefined' && myResumeData[sectionKey] && myResumeData[sectionKey].length > 0) {
        var htmlData = "<div class='cv-board'>";
        
        myResumeData[sectionKey].forEach(function(item) {
          htmlData += "<div class='cv-item'>";
          
          if(item.logo !== "") {
            htmlData += "<img src='" + item.logo + "' class='cv-logo-box'>";
          } else {
            htmlData += "<div class='cv-logo-box'></div>";
          }
          
          htmlData += "<div class='cv-content'><div class='cv-header'>";
          htmlData += "<div><div class='cv-title'>" + item.title + "</div><div class='cv-company'>" + item.company + "</div></div>";
          htmlData += "<div class='cv-date'>" + item.date + "</div>";
          htmlData += "</div>"; 
          
          item.description.forEach(function(desc) {
            htmlData += "<p class='cv-desc'>" + desc + "</p>";
          });
        ...
        // عدد ۱۴۵ یعنی متن دقیقا بعد از لوله‌ی خروج، روی سیاهیِ مطلق قرار می‌گیرد
        pushPreText(htmlData, 145, 40);
      }
    });
  };
  ...
  map.locs = [
    new Location(0, true),
    new Location(0, exitPipeVert), new Location(1),
    new Location(0, exitPipeVert), new Location(2),
    new Location(0, exitPipeVert), new Location(3),
    new Location(0, exitPipeVert), new Location(4),
    new Location(0, exitPipeVert), new Location(5),
    new Location(0, exitPipeVert), new Location(6),
    new Location(0, false)
  ];
  ...
  map.areas = [
    new Area("Overworld", function() {
      ...
      greeter += "    <span style='font-size:81px;line-height:96px'>Mili BROS.</span>";
      greeter += "  </p>";
      greeter += "</div>";
      greeter += "<div id='boo' style='text-align:right;color:#ffcccc;margin-top:-7px;width:350px;height:35px;'>&copy;1995 Milad </div>";
      ...
      //توسط میلاد
    //  pushPrePipe(224, 0, 16, false, 2, 1);
    //  pushPrePipe(304, 0, 24, false, 4, 3);
    //  pushPrePipe(368, 0, 32, false, 6, 5);
    //  pushPreThing(Goomba, 340, 8);
    //  pushPreThing(Goomba, 412, 8);
    //  pushPreThing(Goomba, 422, 8);
    //  pushPrePipe(456, 0, 32, false, 8, 7);
      //
      //توسط میلاد - اضافه کردن لوله‌ها و متن‌های رزومه روی آن‌ها
      pushPrePipe(224, 0, 16, false, 2, 1);
      pushPreText("<div class='cv-float-text' style='text-shadow: 2px 2px 0px black; font-size: 15px; display: flex; flex-direction: column; align-items: center; text-align: center; line-height: 1.0;'>About Me<br>⌄</div>", 216, -40);
      pushPrePipe(304, 0, 24, false, 4, 3);
      pushPreText("<div class='cv-float-text' style='text-shadow: 2px 2px 0px black; font-size: 15px; display: flex; flex-direction: column; align-items: center; text-align: center; line-height: 1.0;'>Experience I<br>⌄</div>", 289, -31);

      pushPrePipe(368, 0, 32, false, 6, 5);
      pushPreText("<div class='cv-float-text' style='text-shadow: 2px 2px 0px black; font-size: 15px; display: flex; flex-direction: column; align-items: center; text-align: center; line-height: 1.0;'>Experience II<br>⌄</div>", 352, -23);
      ...
      pushPrePipe(456, 0, 32, false, 8, 7);
      pushPreText("<div class='cv-float-text' style='text-shadow: 2px 2px 0px black; font-size: 15px; display: flex; flex-direction: column; align-items: center; text-align: center; line-height: 1.0;'>Experience III<br>⌄</div>", 438, -23);
      //
      //
   ...
   ///pushPrePipe(1304, 0, 16, false, false);
      ///جایگزیم توسظ میلاد با بالایی
      pushPrePipe(1304, 0, 16, false, 10, 9);
      pushPreText("<div class='cv-float-text' style='text-shadow: 2px 2px 0px black; font-size: 15px; display: flex; flex-direction: column; align-items: center; text-align: center; line-height: 1.0;'>Skills<br>⌄</div>", 1300, -38);
...
// pushPrePipe(1432, 0, 16);
     //جایگزین با بالایی توسط میلاد
     pushPrePipe(1432, 0, 16, false, 12, 11);
// لوله بازگشت به نقطه شروع (حلقه بی‌نهایت)
     pushPrePipe(1432, 0, 16, false, 13);
     pushPreText("<div class='cv-float-text' style='text-shadow: 2px 2px 0px black; font-size: 15px; display: flex; flex-direction: column; align-items: center; text-align: center; line-height: 1.0;'>Restart<br>⌄</div>", 1426, -38);     
    ...
    //  pushPreThing(PipeVertical, 120, 88, 88);
    //})
    //جایگزین شده با بالایی توسط میلاد
createUnderworld(2, 1, "skills"),
    createUnderworld(4, 3, "experience"),
    createUnderworld(6, 5, "projects"),
    createUnderworld(8, 7, "education"),
    createUnderworld(10, 9, "contact"),
    createUnderworld(12, 11, "about")
...
/* خذف نقشه‌ها توسط میلاد بعدا میتونی کل این کامنت‌رو خذف کنی تا سبک‌تر شه
